...
Served by GitHub Pages from the `gh-pages` branch, root folder, live at **https://bulleyball.club/**.
`CNAME` holds the custom domain and `.nojekyll` stops Jekyll from touching the asset folders. Every
asset path in `index.html` is relative, so the site works unchanged at a sub-path or at a domain
root; `og:url` and `og:image` are the exception and are absolute, because scrapers do not reliably
resolve relative URLs — if the domain ever changes, those two need changing with it.

DNS for the apex points at Pages: A records to `185.199.108.153`, `185.199.109.153`,
`185.199.110.153`, `185.199.111.153`, with a `CNAME` for `www` to `x1001000.github.io`. Tick
**Enforce HTTPS** in Settings → Pages once the certificate has issued.

Any static host (Cloudflare Pages, Netlify) works with no configuration.
